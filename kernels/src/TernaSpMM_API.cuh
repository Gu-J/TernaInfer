...
#include "./SpMM/SpMM_Kernel.cuh"
#include "./SpMM/Reduction_Kernel.cuh"
#include "./GEMM/gemm_scale_ws.h"
    ...
    switch (N_Global) {
        ...
        default:
            ...
            mySpMM_SplitK_Kernel_Ex_bitmap_v3<TilingConfigBitmapV3<4, 1, 4>>(stream,
                ...
                printf("MM_Sparse_API Error: Unsupported N dimension %d! N needs to be padded to a power of two and >= 8!\n", N_Global);
...
void cutlass_int8_gemm_with_epilogue_visitor(
                            cudaStream_t   stream,
                            int8_t*  A,
                            int8_t*  B,  
                            __nv_bfloat16* C,
                            ...
                            int ws_num)
{
    ...
    using GemmScaleWs = cutlass::GemmScaleWs<>;

    GemmScaleWs::TensorRefA ref_A(
        A,
        LayoutA::packed({M,K}));

    GemmScaleWs::TensorRefB ref_B(
        B,
        LayoutB::packed({K,N}));

    GemmScaleWs::TensorRefD ref_C(
        (cutlass::bfloat16_t*)C,
        LayoutOutput::packed({M,N}));

    GemmScaleWs::Arguments args(
        {M,N,K},
        ref_A,
        ref_B,
        ref_C,
        (cutlass::bfloat16_t*)s,
                            ...
                            int ws_num)
{
    ...
    cutlass_int8_gemm_with_epilogue_visitor(stream, B, (int8_t*)Reduction_Workspace, C, N_Global, M_Global, K_Global, s, ws, ws_num);
